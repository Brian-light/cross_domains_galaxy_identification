...
from src.network import DeepMerge, AdversarialNetwork, AdversarialLayer, AdvDeepMerge
...
def main():
    ...
    # sets up modle on cuda is available
    if torch.cuda.is_available():
        model = AdvDeepMerge().cuda()
    ...
    if params['data']['do_transfer_model'] == False:
        model = AdvDeepMerge().to(torch.device(device_string))
    else:
    ...
    gradient_reverse_layer = AdversarialLayer(high_value=1.0)   
    ...
    parameter_list = [{"params":model.parameters(), "lr_mult":1, 'decay_mult':2}]
    optim_params = {"lr":params['exec']['lr'], "betas":(0.7,0.8), "weight_decay": 0.0001, "amsgrad":True, "eps":1e-8}
    optimizer = optim.Adam(parameter_list, **(optim_params))
    ...
    if training_mode == 1:
        print("Training mode: Adversarial + fisher + EM")
    else:
        print("Training mode: Adversarial only")
    losses, model = train(model,
                   gradient_reverse_layer,
                   optimizer,
                   params['exec']['lambda_tl'],
                   num_epochs,
                   training_mode,
                   source_train_dl,
                   source_valid_dl,
    ...
    print('Accuracy: {:.2f}%    Balanced Accuracy: {:.2f}%    Precision: {:.2f}    Recall: {:.2f}    F1: {:.2f}    Brier_score: {:.2f}    AU: {:.2f}%'.format(s_results[0] * 100, s_results[1] * 100, s_results[2] * 100, s_results[3], s_results[4], s_results[5], s_results[6] * 100))
    print('Target domain results:')
    print('Accuracy: {:.2f}%    Balanced Accuracy: {:.2f}%    Precision: {:.2f}    Recall: {:.2f}    F1: {:.2f}    Brier_score: {:.2f}    AU: {:.2f}%'.format(t_results[0] * 100, t_results[1] * 100, t_results[2] * 100, t_results[3], t_results[4], t_results[5], t_results[6] * 100))
